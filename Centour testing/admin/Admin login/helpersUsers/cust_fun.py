...
def find_element_by_name(driver, data_list):
    for data in data_list:
        driver.find_element(By.NAME,(data["field"])).send_keys(data["value"])
...
def find_element_by_tag_name (driver,data_list):
    for data in data_list:
        driver.find_element(By.TAG_NAME(data["field"])).send_keys(data["value"])

def find_element_by_link_text(driver,data_list):
    pass


def find_elements_by_name(driver, data_list,click_btn=None):
    for data in data_list:
        driver.find_element(By.NAME(data ["field"])).click(data["value"])